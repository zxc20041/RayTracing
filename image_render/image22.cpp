...
color image22_ray_color(const ray& r, const hittable& world, int depth) {
    ...
    if (world.hit(r, interval(0.001f, infinity), rec)) {
        ...
        if (rec.mat->scatter(r, rec, attenuation, scattered))
            return attenuation * image22_ray_color(scattered, world, depth-1);
        return color(0.0f, 0.0f, 0.0f);
    }
...
render_scene create_image22_scene() {
    render_scene scene;
    ...
    scene.cam.defocus_angle = 10.0f;
    scene.cam.focus_dist    = 3.4f;
    ...
    scene.ray_color_with_depth = image22_ray_color;
    return scene;
}

namespace {
struct auto_reg_t {
    auto_reg_t() { register_renderer({"image22", create_image22_scene}); }
} auto_reg;
} // namespace
