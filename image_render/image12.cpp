...
color image12_ray_color(const ray& r, const hittable& world, int depth) {
...
render_scene create_image12_scene() {
    render_scene scene;
    ...
    scene.ray_color_with_depth = image12_ray_color;
    return scene;
}

namespace {
struct auto_reg_t {
    auto_reg_t() { register_renderer({"image12", create_image12_scene}); }
} auto_reg;
} // namespace
