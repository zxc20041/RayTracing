...
color image16_ray_color(const ray& r, const hittable& world, int depth) {
    ...
    if (world.hit(r, interval(0.001f, infinity), rec)) {
        ...
        if (rec.mat->scatter(r, rec, attenuation, scattered))
            return attenuation * image16_ray_color(scattered, world, depth-1);
        return color(0.0f, 0.0f, 0.0f);
    }
...
render_scene create_image16_scene() {
    render_scene scene;
    ...
    scene.ray_color_with_depth = image16_ray_color;
    return scene;
}

namespace {
struct auto_reg_t {
    auto_reg_t() { register_renderer({"image16", create_image16_scene}); }
} auto_reg;
} // namespace
