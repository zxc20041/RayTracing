...
color image8_ray_color(const ray& r, const hittable& world, int depth) {
    ...
    if (world.hit(r, interval(0.0f, infinity), rec)) {
        ...
        return 0.5f * image8_ray_color(ray(rec.p, direction), world, depth - 1);
    }
...
render_scene create_image8_scene() {
    ...
    scene.ray_color_with_depth = image8_ray_color;
    return scene;
}

namespace {
struct auto_reg_t {
    auto_reg_t() { register_renderer({"image8", create_image8_scene}); }
} auto_reg;
} // namespace
